...
const AddInventory = () => {
  ...
  const validateForm = () => {
    ...
    const requiredFields = [
      'itemId', 'itemName', 'category', 'description', 'unitOfMeasure', 'quantityInStock', 'reorderLevel',
      'reorderQuantity', 'supplierId', 'costPrice', 'stockStatus','brand', 'locationInStore',  'lastRestockedDate', 'expirationDate'
    ];
    
    requiredFields.forEach(field => {
      if (!inventoryData[field]) newErrors[field] = `${field.replace(/([A-Z])/g, ' $1')} is required.`;
    });
